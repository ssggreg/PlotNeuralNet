...
def block_Unconv_2(name, botton, top, s_filer=256, n_filer=64, offset="(1,0,0)", size=(32, 32, 3.5), opacity=0.5):
    return [
        to_ConvRes(name='ccr_res_{}'.format(name), offset=offset, to="({}-east)".format(botton),
                   s_filer=str(s_filer), n_filer=str(n_filer[0]), width=size[2], height=size[0], depth=size[1],
                   opacity=opacity),
        to_ConvRes(name='{}'.format(top), offset="(0,0,0)", to="(ccr_res_{}-east)".format(name),
                   s_filer=str(), n_filer=str(n_filer[1]), width=size[2], height=size[0], depth=size[1],
                   opacity=opacity),
        to_connection(
            "{}".format(botton),
            "ccr_res_{}".format(name)
